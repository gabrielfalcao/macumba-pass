...
class StartResponse:
    ...
    def __call__(self, status, headers, exc_info=None):
        ...
        logger.info('awsgi.StartResponse.__call__: {}'.format(exc_info))
        return self.write

    def write(self, stuff):
        logger.info('awsgi.StartResponse.write: {}'.format(repr(stuff)))
        if not isinstance(stuff, (str, bytes)):
            logger.warning('not string awsgi.StartResponse.write: {}'.format(repr(stuff)))
            try:
